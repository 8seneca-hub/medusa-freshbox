import { ModuleResolution } from "@freshbox-medusa/types"
...
describe("modules loader", () => {
  ...
  it("should throw an error if the default package isn't found and the module is required", async () => {
    ...
    const moduleResolutions: Record<string, ModuleResolution> = {
      testService: {
        resolutionPath: "@freshbox-medusa/testService",
        definition: {
          key: "testService",
          defaultPackage: "@freshbox-medusa/testService",
          label: "TestService",
          isRequired: true,
          defaultModuleDeclaration: {
            scope: MODULE_SCOPE.INTERNAL,
    ...
    try {
      ...
      expect(err.message).toEqual(
        `Make sure you have installed the default package: @freshbox-medusa/testService`
      )
    }
  })
