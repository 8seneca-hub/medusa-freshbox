import type { MedusaAppLoader } from "@freshbox-medusa/framework"
import { logger } from "@freshbox-medusa/framework/logger"
import { Logger, MedusaContainer } from "@freshbox-medusa/framework/types"
import {
...
export async function initDb() {
  const { pgConnectionLoader } = await import("@freshbox-medusa/framework")
...
async function loadCustomLinks(directory: string, container: MedusaContainer) {
  ...
  const { LinkLoader } = await import("@freshbox-medusa/framework")
