...
describe("Local notification provider", () => {
  ...
  it("sends logs to the console output with the notification details", async () => {
    ...
    expect(logSpy).toHaveBeenCalledWith(
      `Attempting to send a notification to: 'test@freshbox-medusa.com' on the channel: 'email' with template: 'some-template' and data: '{\"username\":\"john-doe\"}'`
    )
  })
})
